(function () {
    ...
    function handleDragStart(event) {
        ...
        const closestColumn = card.closest('.vendas-kanban-column, .kanban-column');
        dragState.originStatus = closestColumn && closestColumn.dataset ? closestColumn.dataset.status : null;
    ...
    async function handleDrop(event) {
        ...
        const targetStatus = column && column.dataset ? column.dataset.status : null;
        const cardId = dragState.cardId || (event.dataTransfer ? event.dataTransfer.getData('text/plain') : null);
    ...
    async function handleCreateSubmit(event) {
        ...
        const payload = {
            cooperativa: getFormValue(data, 'cooperativa'),
            tipoVeiculo: getFormValue(data, 'tipoVeiculo'),
            placa: getFormValue(data, 'placa'),
            marca: getFormValue(data, 'marca'),
            anoModelo: getFormValue(data, 'anoModelo'),
            modelo: getFormValue(data, 'modelo'),
            nomeContato: getFormValue(data, 'nomeContato'),
            email: getFormValue(data, 'email'),
            celular: getFormValue(data, 'celular'),
            estado: getFormValue(data, 'estado'),
            cidade: getFormValue(data, 'cidade'),
            origemLead: getFormValue(data, 'origemLead'),
            veiculoTrabalho: data.get('veiculoTrabalho') === 'on',
            enviarCotacao: data.get('enviarCotacao') === 'on',
            observacoes: getFormValue(data, 'observacoes')
        };
